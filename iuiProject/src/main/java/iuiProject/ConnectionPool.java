 ...
 * <p>
 * The idber of <code>Connection</code>s a <code>ConnectionPool</code> object
...
public final class ConnectionPool {
	...
	// Maximum idber of concurrent connections allowed.
	private int maxCons = 0;

	// The idber of connection that have been created.
	 ...
	 * @param initialCons The idber of connections to create now.
	 * @param maxCons     The maximum idber of connections allowed. A value <= 0
	 ...
	 **/
	private ConnectionPool(String url, String user, String password, int initialCons, int maxCons, boolean block,
		...
		while (idCons < initialCons) {
			addConnection();
		}
	}

	public static ConnectionPool getInstance(String url, String user, String password, int initialCons, int maxCons,
			boolean block, long timeout) throws SQLException {
		if (pool == null)
			pool = new ConnectionPool(url, user, password, initialCons, maxCons, block, timeout);
		return pool;
	...
	public synchronized void closeAll() {
		...
		Enumeration<Connection> cons = ((Vector) free.clone()).elements();
	 ...
	 *
	 * @return A connection
	 * @exception ConnectionPoolException if the maximum idber of allowed
		...
		if (free.isEmpty()) {
			...
			if (maxCons <= 0 || idCons < maxCons) {
				...
				try {
					...
					} while (timeout >= 0 && free.isEmpty() && maxCons > 0 && idCons >= maxCons);
				...
				if (free.isEmpty()) {
					 ...
					 * they closed, so the free list is empty but idCons is now < maxCons and we can
				...
				throw new SQLException("Maximum idber of allowed connections reached");
			}
		}
	 ...
	 * specifies the maximum idber of <code>Connections</code> that can be allocated
	 * from the pool at any one time.
	 *
